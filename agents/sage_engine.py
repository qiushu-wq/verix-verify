...
STRUCTURE_TEMPLATES = {
    ...
    'beta': {
        ...
        'patterns': [
            {
                'id': 'beta_proof_explosion',
                'name': '证明搜索空间爆炸',
                'structure': '{搜索算法}在{证明树}中搜索{目标定理}时，{搜索空间}随{深度}{指数增长}。{剪枝策略}可减少{无效分支}但{可能}丢失{非标准证明路径}。',
                'abstract': '搜索空间随问题规模指数增长导致穷举不可行',
                'detection': ['搜索', '证明', 'BFS', 'depth', '节点'],
                'source_solutions': [
                    {'domain': '游戏AI', 'solution': '蒙特卡洛树搜索——用随机采样替代穷举搜索，优先探索最有希望的分支'},
                    {'domain': '优化理论', 'solution': '分支定界法——在搜索空间中剪除不可能包含最优解的区域'},
                ],
            },
            {
                'id': 'beta_type_inference',
                'name': '类型推断歧义',
                'structure': '{类型检查器}在{验证}代码时，{隐式类型}与{显式标注}产生{冲突}。{类型推断}需要在{多个候选类型}中选择{最合适的}，但{选择标准}可能{不明确}。',
                'abstract': '隐式类型与显式类型的冲突导致验证失败',
                'detection': ['类型', 'type', 'match', 'mismatch'],
                'source_solutions': [
                    {'domain': '编程语言设计', 'solution': '类型注解——要求关键位置显式声明类型，减少推断歧义'},
                ],
            },
        ],
    },
    'epsilon': {
        ...
        'patterns': [
            # ── 原有 ──
            {'id': 'epsilon_predicate_ambiguity', 'name': '谓词映射歧义',
             'structure': '自然语言谓词到知识库属性的映射歧义',
             'abstract': '自然语言谓词到知识库属性的映射歧义',
             'detection': ['P19', 'P27', '出生'],
             'source_solutions': [{'domain': '翻译学', 'solution': '一词多义消歧——用上下文约束缩小候选词义范围'}]},
            # ── 新增 ──
            {
                'id': 'epsilon_coverage_gap',
                'name': '知识覆盖率缺口',
                'structure': '{知识库}的{覆盖率}随{实体数量}{指数下降}。{高频实体}被{充分覆盖}，但{长尾实体}的{覆盖密度}极低。',
                'abstract': '知识库覆盖率随实体规模增长而衰减',
                'detection': ['覆盖率', '实体', '知识库', 'kb'],
                'source_solutions': [
                    {'domain': '数据库索引', 'solution': '分层存储——热数据全索引，冷数据压缩存储按需查询'},
                ],
            },
            {
                'id': 'epsilon_semantic_gap',
                'name': '谓词语义偏差',
                'structure': '{自然语言谓词}的{语义范围}与{知识库属性}的{定义域}{不匹配}。{同一个谓词}可映射到{多个KB属性}，导致{验证结果}依赖{映射选择}。',
                'abstract': '自然语言到知识库属性的映射歧义',
                'detection': ['谓词', '映射', '歧义', '属性', '知识库'],
                'source_solutions': [
                    {'domain': '翻译学', 'solution': '上下文消歧——利用主语类型约束候选谓词范围'},
                ],
            },
        ],
    },
